...
const RPC_NAME = "get_tet_ganit_vigyan_questions"; // <-- બદલ્યું
...
export default function TETGanitVigyanSetPage() {
  ...
  useEffect(() => {
    if (!setNo || setNo < 1 || setNo > 8) router.replace("/exams/tet-ganit-vigyan");
  }, [setNo, router]);
  ...
  async function startNewAttempt() {
    try {
      ...
      if (raw.length === 0) {
        alert("No TET Ganit Vigyan questions found.");
        return;
      }
      ...
      console.error(e);
      alert("Error loading TET Ganit Vigyan questions.");
    } finally {
      setLoading(false);
    }
  }
  ...
  const onSubmit = () => {
    if (answers.some((a) => a === -1) && !confirm("કેટલાક પ્રશ્નો અનઉત્તરિત છે. ફરી પણ સબમિટ કરવો છે?")) return;
    setSubmitted(true);
    window.scrollTo({ top: 0, behavior: "smooth" });
  };
  ...
  if (!started) {
    return (
      <div className="max-w-4xl mx-auto py-8">
        <h1 className="text-3xl font-bold mb-2">TET – Ganit Vigyan (Set {setNo})</h1>
        <p className="text-gray-700 mb-6">
          આ છીએ ટેસ્ટ સેટમાં કુલ {QUESTION_COUNT} પ્રશ્નો. દરેક પ્રયત્ને Questions અને Options ઓટોમેટિકલી શફલ થાય છે. નીચેના બટનથી ટેસ્ટ શરૂ કરો.
        </p>
        <button
