...
const RPC_NAME = "get_tet_bhasha_questions";
...
export default function TETBhashaSetPage() {
  ...
  useEffect(() => {
    if (!setNo || setNo < 1 || setNo > 8) router.replace("/exams/tet-bhasha");
  }, [setNo, router]);
  ...
  async function startNewAttempt() {
    try {
      ...
      if (raw.length === 0) {
        alert("No TET Bhasha questions found.");
        return;
      }
      ...
      console.error(e);
      alert("Error loading TET Bhasha questions.");
    } finally {
      setLoading(false);
    }
  }
