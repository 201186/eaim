...
const RPC_NAME = "get_gk_questions"; // ⬅️ new RPC
...
export default function GKSetPage() {
  ...
  useEffect(() => {
    if (!setNo || setNo < 1 || setNo > 8) router.replace("/exams/general-knowledge");
  }, [setNo, router]);
  ...
  async function startNewAttempt() {
    try {
      ...
      if (raw.length === 0) {
        alert("No GK questions found. Please add some in Supabase.");
        return;
      }
      ...
      console.error(e);
      alert("Error loading GK questions.");
    } finally {
      setLoading(false);
    }
  }
  ...
  if (!started) {
    return (
      <div className="max-w-4xl mx-auto py-8">
        <h1 className="text-3xl font-bold mb-2">GK – Set {setNo}</h1>
        <p className="text-gray-700 mb-6">આ સેટમાં {QUESTION_COUNT} પ્રશ્નો હશે.</p>
        <button
  ...
  if (submitted) {
    ...
    return (
      <div className="max-w-4xl mx-auto py-8">
        <h1 className="text-3xl font-bold mb-2">GK – Result (Set {setNo})</h1>
